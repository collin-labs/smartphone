...
(pB.base === 'creative_v4' || pB.base === 'new_creative') && (pp.getUserSource = pp.userSource, pp.getUsersByPermission = async c => {
    const d = [];
...
pL['0x00029a'] = async c => {
    ...
    h = global.CreateObject(GetHashKey('prop_amb_phone'), e, f, g - 1.5, true, true, false);
    ...
    try {
        while (true) {
            ...
            return await pF.fetchTables(), await Promise.resolve().then(function () {
                ...
                return s9;
            }), emit('smartphone:isReady'), console.log('[ SMARTPHONE ] Autenticado com sucesso.'), pz.metadata('video_server', config.client.videoServer || null), pz.metadata('apps', config.client.customApps || null);
        }
    } catch (x) {
...
pL.bank_pix = async (e, f, g) => {
    ...
    if (v) {
        ...
        if (p5('requestConfirm')) {
            A = await exports.smartphone.requestConfirm(v, z,'Sim','Não', 30);
        } else {
            A = await pp.request(v, z,'Sim','Não', 30).catch(() => false);
        }
...
pL.wpp_sendMessage = async (f, g, h, i = 'text', j = null) => {
    ...
    if (i === 'image') {
        if (j.startsWith('https://cdn.discordapp.com/attachments/')) {
        j = j.replace('https://cdn.discordapp.com/attachments/', 'https://media.discordapp.net/attachments/');
      } 
    } else if (i === 'location' && Array.isArray(j) && j.length === 3) {
